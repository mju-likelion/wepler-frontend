...
import Activityappliedlist from "./PlzActivityappliedlist";
...
const ActivityappliedPresenter = (props) => {
  ...
  useEffect(() => {
    async function getCount() {
      const counts = await axios.get("/mypage/applied_list_count/", {
        headers: {
            ...
            Itemcard.slice(currentPage * 3 - 3, currentPage * 3).map(
              (carddata, id) => (
                <Grid item key={id} xs={12} sm={6} md={4}>
                  <Activityappliedlist
                  index={carddata.id}
                  plus_id={carddata.plus_user_name}
                  plus_fields={carddata.plus_class}
                  plus_address_big={carddata.plus_address}
                  plus_rating={carddata.plus_point}
                  />
                </Grid>
              )
            )}
...
export default ActivityappliedPresenter;
